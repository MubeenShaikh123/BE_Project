...
#
lbl = tk.Label(root, text="Lungs Cancer Detection", font=('times', 25,' bold '), height=1, width=70,fg="Black") #bg="Black",fg="white"
lbl.place(x=60, y=20)


frame_alpr = tk.LabelFrame(root, text=" --Process-- ", width=220, height=350, bd=5, font=('times', 14, ' bold '),bg="#808080")
...
#    global fn
    ...
    if fn!="":
        ...
        model = load_model('Lungs_model.h5')
        ...
        dental=np.argmax(prediction)
        print(dental)
        
        
        
        if dental== 0:
            Cd="NO CENCER DETECTED"
        elif dental== 1:
            Cd="CENCER DETECTED"
...
def openimage():
    ...
    global fn
    fileName = askopenfilename(initialdir='D:\lungs Detection\Dataset', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
...
## Train button of using only train perpose 
